...
def calc_model_new(p, inputs_dict, extra_arg_kw=None):
...
def main():
    ...
    runner.run_mcmc(load_backend=False, nsteps=500, progress=True)


    "Running from previous MCMC"

    runner.run_mcmc(load_backend=True, nsteps=500, progress=True)


    "Visualizing"

    runner.plot_corner(discard=40)
    ...
    outputs_hist = runner.get_outputs_history(thin=1, discard=0)
    outputs_hist_flat = runner.get_outputs_history(flat=True, thin=1, discard=0)

    runner.plot_mcmc_outputs(thin=30, save_no_show=True)
